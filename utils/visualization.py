...
def write_ply(verts, colors, indices, output_file):
    mesh_write = TriangleMesh()
    mesh_write.vertices = Vector3dVector(vertices)
    mesh_write.vertex_colors = Vector3dVector(colors)
    mesh_write.triangles = Vector3iVector(indices)
    write_triangle_mesh(output_file, mesh_write, write_ascii=True)


def read_ply(verts, colors, indices, input_file):
    mesh = read_triangle_mesh(input_file)
        ...
        for y in range(mask.shape[1]):
            for x in range(mask.shape[0]):
                if mask[x, y, z] > 0:
                    ...
                    box_verts, box_color, box_ind = make_voxel_mesh(box_min, box_max, np.array(create_color_palette()[int(mask[x,y,z]%41)]))
                    cur_num_verts = len(verts)
...
if __name__ == '__main__':
    mask = np.ones((3, 3, 3))
    voxel2obj(mask, 'mask.obj')

    bbox = np.array([[1,2,3,6,7,8,2]])
    bbox2obj(bbox, 'box.obj')
